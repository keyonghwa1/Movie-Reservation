...
public class Paging {
	...
	private int totalCount; // DB에서 조회(미리 조회해서 넘길예정)
	...
	public Paging() { }
	// 페이지 객체를 완성하기 위해 필요한 값들을 전달받는다. (3개 항목은 전달 받아야 나머지를 계산할 수 있다.)
	...
	private void calc() {
		...
		// 그런데 요청 페이지가 전체 페이지 수보다 크면, 그냥 마지막 페이지로 설정한다.(잘못된 요청)
		if( currentPage > totalPage ) {
			currentPage = totalPage;
		}
		
		// currentPage가 2, 항목 수가 10이ㅏㄹ면 --> 11
		startNo = (currentPage - 1) * pageSize + 1;
		
		// startNo가 11, 항목 수가 10이라면 --> 20
		endNo = startNo + pageSize - 1;
		// 2 페이지에서는 11~20 순서의 10개 항목을 보여주고 싶다!
		
		// 마지막 페이지에서는 endNo가 전체 개수보다 클 수도 있다.
		// 13개 있는데, 2페이지 요청 => 11~20이 되는데... 14~20은 없다. 그래서 13으로 변경(11~13)
		if( endNo > totalCount ) {
			endNo = totalCount;			
		}
		...
		endPage = startPage + 9;
		// 시작과 끝 페이지를 1~10, 11~20, 21~30형태로 맞추고 싶다.
		
		// 마지막 페이지 번호가 전체 페이지 수보다 클 수도 있다.
